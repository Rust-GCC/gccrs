...
fn deref_mut_field1(x: Own<Point>) {
    let __isize = &mut x.y; // { dg-error ".E0596." "" { target *-*-* } }
}
...
fn deref_extend_mut_field3(x: &mut Own<Point>) {
    ...
    let _x = &mut x.x;
    let _y = &mut x.y; // { dg-error ".E0499." "" { target *-*-* } }
    use_mut(_x);
}
...
fn deref_mut_method1(x: Own<Point>) {
    x.set(0, 0); // { dg-error ".E0596." "" { target *-*-* } }
}
...
fn assign_method1<'a>(x: Own<Point>) {
    *x.y_mut() = 3; // { dg-error ".E0596." "" { target *-*-* } }
}
