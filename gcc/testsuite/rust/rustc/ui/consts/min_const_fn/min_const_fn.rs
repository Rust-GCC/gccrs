...
impl<T> Foo<T> {
    ...
    const fn into_inner(self) -> T { self.0 } // { dg-error ".E0493." "" { target *-*-* } }
...
impl<'a, T> Foo<T> {
    ...
    const fn into_inner_lt(self) -> T { self.0 } // { dg-error ".E0493." "" { target *-*-* } }
...
impl<T: Sized> Foo<T> {
    ...
    const fn into_inner_s(self) -> T { self.0 } // { dg-error ".E0493." "" { target *-*-* } }
...
static BAR: u32 = 42;
const fn foo25() -> u32 { BAR } // { dg-error ".E0013." "" { target *-*-* } }
...
// { dg-error ".E0723." "" { target *-*-* } .-1 }
// { dg-error ".E0723." "" { target *-*-* } .-2 }
// { dg-error ".E0723." "" { target *-*-* } .-3 }
