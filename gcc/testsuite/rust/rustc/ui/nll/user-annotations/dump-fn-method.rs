...
fn main() {
    ...
    let x = foo::<&'static u32>; // { dg-error "" "" { target *-*-* } }
    x(&22);

    // Here: we only want the `T` to be given, the rest should be variables.
    //
    // (`T` refers to the declaration of `Bazoom`)
    let x = <_ as Bazoom<u32>>::method::<_>; // { dg-error "" "" { target *-*-* } }
    x(&22, 44, 66);
    ...
    let x = <u8 as Bazoom<&'static u16>>::method::<u32>; // { dg-error "" "" { target *-*-* } }
    x(&22, &44, 66);
    ...
    let y = 22_u32;
    y.method::<u32>(44, 66); // { dg-error "" "" { target *-*-* } }
