...
fn with_obj(c: &dyn CompareToInts) -> bool {
    c.same_as(22) // { dg-error ".E0277." "" { target *-*-* } }
}
...
fn with_ufcs1<C:CompareToInts>(c: &C) -> bool {
    CompareToInts::same_as(c, 22) // { dg-error ".E0277." "" { target *-*-* } }
}

fn with_ufcs2<C:CompareToInts>(c: &C) -> bool {
    CompareTo::same_as(c, 22) // { dg-error ".E0277." "" { target *-*-* } }
}

fn main() {
    assert_eq!(22_i64.same_as(22), true); // { dg-error ".E0277." "" { target *-*-* } }
}
