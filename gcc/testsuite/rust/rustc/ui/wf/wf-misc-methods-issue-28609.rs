...
fn return_dangling_pointer_inherent(s: S2) -> &u32 {
    let s = s;
    s.transmute_inherent(&mut 42) // { dg-error ".E0515." "" { target *-*-* } }
}
...
fn return_dangling_pointer_binary_op(s: S2) -> &u32 {
    let s = s;
    s << &mut 3 // { dg-error ".E0515." "" { target *-*-* } }
}

fn return_dangling_pointer_method(s: S2) -> &u32 {
    let s = s;
    s.shl(&mut 3) // { dg-error ".E0515." "" { target *-*-* } }
}

fn return_dangling_pointer_ufcs(s: S2) -> &u32 {
    let s = s;
    S2::shl(s, &mut 3) // { dg-error ".E0515." "" { target *-*-* } }
}
