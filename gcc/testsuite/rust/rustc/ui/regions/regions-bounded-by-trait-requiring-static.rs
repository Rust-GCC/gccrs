...
fn param_not_ok<'a>(x: &'a isize) {
    assert_send::<&'a isize>(); // { dg-error ".E0477." "" { target *-*-* } }
}

fn param_not_ok1<'a>(_: &'a isize) {
    assert_send::<&'a str>(); // { dg-error ".E0477." "" { target *-*-* } }
}
...
fn box_with_region_not_ok<'a>() {
    assert_send::<Box<&'a isize>>(); // { dg-error ".E0477." "" { target *-*-* } }
}
...
fn unsafe_ok2<'a>(_: &'a isize) {
    assert_send::<*const &'a isize>(); // { dg-error ".E0477." "" { target *-*-* } }
}

fn unsafe_ok3<'a>(_: &'a isize) {
    assert_send::<*mut &'a isize>(); // { dg-error ".E0477." "" { target *-*-* } }
}
