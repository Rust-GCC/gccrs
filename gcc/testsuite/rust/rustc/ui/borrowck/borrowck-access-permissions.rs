...
fn main() {
    ...
    { // borrow of local
        let _y1 = &mut x; // { dg-error ".E0596." "" { target *-*-* } }
        let _y2 = &mut x_mut; // No error
    }

    { // borrow of static
        let _y1 = &mut static_x; // { dg-error ".E0596." "" { target *-*-* } }
    ...
    { // borrow of deref to box
        ...
        let _y1 = &mut *box_x; // { dg-error ".E0596." "" { target *-*-* } }
    ...
    { // borrow of deref to reference
        ...
        let _y1 = &mut *ref_x; // { dg-error ".E0596." "" { target *-*-* } }
    ...
    { // borrow of deref to pointer
        ...
        unsafe {
            let _y1 = &mut *ptr_x; // { dg-error ".E0596." "" { target *-*-* } }
    ...
    { // borrowing mutably through an immutable reference
        ...
        let foo_ref = &foo;
        let _y = &mut *foo_ref.f; // { dg-error ".E0596." "" { target *-*-* } }
    }
}
