...
fn test1(mut f: Box<Foo>, s: String) {
    let p = &mut f[&s];
    let q = &f[&s]; // { dg-error ".E0502." "" { target *-*-* } }
    p.use_mut();
}

fn test2(mut f: Box<Foo>, s: String) {
    let p = &mut f[&s];
    let q = &mut f[&s]; // { dg-error ".E0499." "" { target *-*-* } }
    p.use_mut();
}
...
fn test3(mut f: Box<Bar>, s: String) {
    let p = &mut f.foo[&s];
    let q = &mut f.foo[&s]; // { dg-error ".E0499." "" { target *-*-* } }
    p.use_mut();
}
...
fn test5(mut f: Box<Bar>, s: String) {
    let p = &f.foo[&s];
    let q = &mut f.foo[&s]; // { dg-error ".E0502." "" { target *-*-* } }
    p.use_ref();
}

fn test6(mut f: Box<Bar>, g: Foo, s: String) {
    let p = &f.foo[&s];
    f.foo = g; // { dg-error ".E0506." "" { target *-*-* } }
    p.use_ref();
}

fn test7(mut f: Box<Bar>, g: Bar, s: String) {
    let p = &f.foo[&s];
    *f = g; // { dg-error ".E0506." "" { target *-*-* } }
    p.use_ref();
}
