...
fn const_index_and_subslice_from_end_err(s: &mut [i32; 4]) {
    ...
    let [ref mut tail @ .., _] = *s; // { dg-error ".E0502." "" { target *-*-* } }
    nop(&[first, second]);
    nop_subslice(tail);
}
...
fn subslices_overlap(s: &mut [i32; 4]) {
    let [_,  ref s1 @ ..] = *s;
    let [ref mut s2 @ .., _, _] = *s; // { dg-error ".E0502." "" { target *-*-* } }
    nop_subslice(s1);
    nop_subslice(s2);
}
