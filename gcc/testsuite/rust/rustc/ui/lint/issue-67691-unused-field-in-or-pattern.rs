...
pub fn with_ref(x: MyEnum) {
    ...
    match x {
        A { i, ref j } | B { i, ref j } => { // { dg-error "" "" { target *-*-* } }
            println!("{}", i);
        }
    }
}
...
pub fn inner_no_ref(x: Option<MyEnum>) {
    ...
    match x {
        Some(A { i, j } | B { i, j }) => { // { dg-error "" "" { target *-*-* } }
            println!("{}", i);
        }
...
pub fn inner_with_ref(x: Option<MyEnum>) {
    ...
    match x {
        Some(A { i, ref j } | B { i, ref j }) => { // { dg-error "" "" { target *-*-* } }
            println!("{}", i);
        }
...
pub fn mixed_no_ref(x: MixedEnum) {
    match x {
        MixedEnum::A { i } | MixedEnum::B(i) => { // { dg-error "" "" { target *-*-* } }
            println!("match");
        }
    }
}
...
pub fn mixed_with_ref(x: MixedEnum) {
    match x {
        MixedEnum::A { ref i } | MixedEnum::B(ref i) => { // { dg-error "" "" { target *-*-* } }
            println!("match");
        }
    }
}
