...
PatternChecker::visit (MatchExpr &expr)
{
  ...
  compute_match_usefulness (&tyctx, scrutinee_ty, expr);
}
...
Constructor::is_covered_by (const Constructor &o) const
{
  ...
  switch (kind)
    {
      case VARIANT: {
	rust_assert (kind == VARIANT);
	return variant_idx == o.variant_idx;
      }
      break;
      case INT_RANGE: {
	rust_assert (kind == INT_RANGE);
...
PlaceInfo::specialize (const Constructor &c) const
{
  switch (c.get_kind ())
    {
    case Constructor::WILDCARD:
      case Constructor::INT_RANGE: {
	return {};
      }
      break;
    case Constructor::STRUCT:
      case Constructor::VARIANT: {
				       ...
				       Matrix &matrix)
{
  ...
  // FIXME: correct?
  rust_assert (!matrix.get_rows ().empty ());
...
// entry point for computing match usefulness and exhaustiveness
void
compute_match_usefulness (Resolver::TypeCheckContext *ctx,
