					       ...
					       location_t pratt_parsed_loc)
{
  ...
  return std::unique_ptr<AST::ReturnExpr> (
    new AST::ReturnExpr (std::move (returned_expr.value ()),
					       ...
					       location_t pratt_parsed_loc)
{
  ...
  if (lexer.peek_token ()->get_id () != ELSE)
	...
	std::move (match_arm_patterns), std::move (scrutinee_expr.value ()),
  ...
  AST::AttrVec outer_attrs, tl::optional<AST::LoopLabel> label)
{
  ...
  return std::unique_ptr<AST::WhileLetLoopExpr> (
    new AST::WhileLetLoopExpr (std::move (predicate_patterns),
  ...
  const_TokenPtr tok, AST::AttrVec outer_attrs, ParseRestrictions restrictions)
{
  switch (tok->get_id ())
	...
	return std::unique_ptr<AST::NegationExpr> (
	  new AST::NegationExpr (std::move (expr.value ()),
	...
	return std::unique_ptr<AST::NegationExpr> (
	  new AST::NegationExpr (std::move (expr.value ()),
				 NegationOperator::NOT, std::move (outer_attrs),
				 tok->get_locus ()));
      }
    case ASTERISK:
      {
	...
	return std::unique_ptr<AST::DereferenceExpr> (
	  new AST::DereferenceExpr (std::move (expr.value ()),
				    std::move (outer_attrs),
				    tok->get_locus ()));
      }
    case AMP:
    ...
    case TRY:
      ...
      {
	auto try_expr
	  = parse_try_expr (std::move (outer_attrs), tok->get_locus ());
	if (try_expr)
	  return std::move (try_expr.value ());
	else
	  return nullptr;
      }
    case BREAK:
	...
	if (break_expr)
	  return std::move (break_expr.value ());
	else
	  return nullptr;
      }
    case CONTINUE:
      ...
      if (lexer.peek_token ()->get_id () == LET)
	{
	  ...
	  if (!if_let_expr)
	    ...
	    return std::move (if_let_expr.value ());
	}
      else
	{
	  // if expr
	  auto if_expr
	    = parse_if_expr (std::move (outer_attrs), tok->get_locus ());
	  if (if_expr)
	    return std::move (if_expr.value ());
	  else
	    return nullptr;
	}
    case LIFETIME:
      {
	auto labelled_loop_expr
	  = parse_labelled_loop_expr (tok, std::move (outer_attrs));
	if (labelled_loop_expr)
	  return std::move (labelled_loop_expr.value ());
	else
	  return nullptr;
      }
    case LOOP:
      {
	auto loop_expr = parse_loop_expr (std::move (outer_attrs), tl::nullopt,
					  tok->get_locus ());
	if (loop_expr)
	  return std::move (loop_expr.value ());
	else
	  return nullptr;
      }
    case WHILE:
      if (lexer.peek_token ()->get_id () == LET)
	{
	  auto while_let_loop_expr
	    = parse_while_let_loop_expr (std::move (outer_attrs));
	  if (while_let_loop_expr)
	    return std::move (while_let_loop_expr.value ());
	  else
	    return nullptr;
	}
      else
	{
	  auto while_loop_expr
	    = parse_while_loop_expr (std::move (outer_attrs), tl::nullopt,
				     tok->get_locus ());
	  if (while_loop_expr)
	    return std::move (while_loop_expr.value ());
	  else
	    return nullptr;
	}
    case FOR:
      {
	auto for_loop_expr
	  = parse_for_loop_expr (std::move (outer_attrs), tl::nullopt);
	if (for_loop_expr)
	  return std::move (for_loop_expr.value ());
	else
	  return nullptr;
      }
    case MATCH_KW:
	...
	if (match_expr)
	  return std::move (match_expr.value ());
	else
	  return nullptr;
      }
	...
	if (array_expr)
	  return std::move (array_expr.value ());
	else
	  return nullptr;
      }
    case UNSAFE:
      {
	auto unsafe_block_expr
	  = parse_unsafe_block_expr (std::move (outer_attrs),
				     tok->get_locus ());
	if (unsafe_block_expr)
	  return std::move (unsafe_block_expr.value ());
	else
	  return nullptr;
      }
    case BOX:
      {
	auto box = parse_box_expr (std::move (outer_attrs), tok->get_locus ());
	if (box)
