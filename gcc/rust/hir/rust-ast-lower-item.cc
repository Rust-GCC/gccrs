  ...
  if (resolver.translated != nullptr)
    {
      ...
      resolver.mappings.hir.items.insert (resolver.translated);
...
ASTLoweringItem::visit (AST::Module &module)
{
  ...
  mappings.hir.modules.insert (static_cast<Module *> (translated));
}
...
ASTLoweringItem::visit (AST::Function &function)
{
  ...
  for (auto &param : fn->get_function_params ())
    {
      mappings.hir.function_params.insert (&param);
...
ASTLoweringItem::visit (AST::ExternCrate &extern_crate)
{
  ...
  CrateNum num
    = crate_mapping.lookup_crate_name (extern_crate.get_referenced_crate ())
	.value ();
  AST::Crate &crate = mappings.crate.get_ast_crate (num);

  auto saved_crate_num = mappings.crate.get_current_crate ();
  crate_mapping.set_current_crate (num);
