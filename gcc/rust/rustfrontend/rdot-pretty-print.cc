...
typeStringNode (const rdot node)
{
    ...
    if (node != NULL_DOT)
    {
        ...
        {
            if (_no_infer)
                fatal_error (RDOT_LOCATION(node), "gcc-rust has failed to infer a type and cannot continue");
            else
        ...
        case D_IDENTIFIER:
            strcpy (buffer+offset, RDOT_IDENTIFIER_POINTER (node));
            break;

        default:
            fatal_error (RDOT_LOCATION(node), "unhandled type [%s]", RDOT_OPCODE_STR (node));
            break;
        }
    }
...
void dot_pass_dumpPrimitive (FILE * fd, rdot node)
{
    ...
    default:
        fatal_error (RDOT_LOCATION(node),
                     "Unable to dump primitive [%s]",
...
void dot_pass_dumpExprNode (FILE * fd, rdot node)
{
	...
	for (next = init; next != NULL_DOT; next = RDOT_CHAIN (next))
        {
	    gcc_assert (RDOT_TYPE (next) == D_STRUCT_INIT_ARGUMENT);
...
void dot_pass_dump_expr (FILE * fd, rdot node)
{
    ...
    default:
    {
        ...
        default:
            fatal_error (RDOT_LOCATION(node), "unhandled dump [%s]!\n", RDOT_OPCODE_STR (node));
            break;
        }
...
void rdot_pretty_print (const char* out, std::vector<rdot> decls)
{
    FILE * fd = fopen (out, "w");
    if (!fd)
    {
