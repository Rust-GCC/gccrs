...
// helper
extern void rdot_pretty_print (const char*, std::vector<rdot>);


// this is a big global since i am unable to use pure C++11 bison parser
// i need a global to dump all the nodes into the syntax tree
class CompilationUnitAst {
public:
    static CompilationUnitAst& get();

    void addNode(rdot);

    std::vector<rdot> getNodes();

private:
    CompilationUnitAst();
    ~CompilationUnitAst();
    
    std::vector<rdot> m_nodes;
};
