  ...
  const std::vector<std::unique_ptr<GenericParam>> &type_generics)
{
  ...
  auto trait_path = builder.type_path ({builder.get_path_start(), "cmp", trait}, true);

  auto trait_bound
    = builder.trait_bound (builder.type_path ({builder.get_path_start(), "cmp", trait}, true));
...
{
  // Ordering
  auto return_type = builder.type_path ({builder.get_path_start(), "cmp", "Ordering"}, true);
...
DeriveOrd::make_equal ()
{
  std::unique_ptr<Pattern> equal = ptrify (
    builder.path_in_expression ({builder.get_path_start(), "cmp", "Ordering", "Equal"}, true));
...
DeriveOrd::recursive_match (std::vector<SelfOther> &&members)
{
  if (members.empty ())
	...
	builder.path_in_expression ({builder.get_path_start(), "cmp", "Ordering", "Equal"},
				    true));
