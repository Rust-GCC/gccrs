...
namespace Rust
{
  namespace
  {
        ...
        std::vector<std::unique_ptr<AST::MacroInvocation>> &&pending_invocations)
    {
      ...
      case AST::BuiltinMacro::Include:
        path_str = "include";
        break;
      case AST::BuiltinMacro::OptionEnv:
        path_str = "option_env";
        break;
      }
                                             ...
                                             std::unique_ptr<AST::Expr> &node)
    {
      ...
      if (!node || !node->is_literal() || maybe_lit->get_lit_type() != AST::Literal::STRING)
      {
                         ...
                         bool &has_error)
    {
      ...
      while (parser.peek_current_token()->get_id() != last_token_id && parser.peek_current_token()->get_id() != END_OF_FILE)
      {
    ...
    {
      std::string compile_fname = Session::get_instance().linemap->location_file(locus);
  ...
  {
    auto current_file = Session::get_instance().linemap->location_file(invoc_locus);
    auto file_str = AST::SingleASTNode(make_string(invoc_locus, current_file));
    auto str_token = make_token(Token::make_string(invoc_locus, std::move(current_file)));
  ...
  {
    auto current_column = Session::get_instance().linemap->location_to_column(invoc_locus);
                                      ...
                                      AST::MacroInvocData &invoc)
  {
    ...
    auto lit_expr = parse_single_string_literal(invoc.get_delim_tok_tree(), invoc_locus,
                                                invoc.get_expander());
    if (lit_expr == nullptr)
      return AST::Fragment::create_error();

    std::string target_filename = source_relative_path(lit_expr->as_string(), invoc_locus);
  ...
  MacroBuiltin::concat_handler(Location invoc_locus, AST::MacroInvocData &invoc)
  {
    ...
    {
      if (!expr->is_literal() && expr->get_ast_kind() != AST::MACRO_INVOCATION)
      {
        has_error = true;
      ...
      if (literal->get_lit_type() == AST::Literal::BYTE || literal->get_lit_type() == AST::Literal::BYTE_STRING)
      {
        has_error = true;
  ...
  MacroBuiltin::env_handler(Location invoc_locus, AST::MacroInvocData &invoc)
  {
    ...
    {
      if (!(lit_expr = try_extract_string_literal_from_fragment(invoc_locus,
                                                                expanded_expr[0])))
      {
        return AST::Fragment::create_error();
      }
    ...
    {
      if (!(error_expr = try_extract_string_literal_from_fragment(invoc_locus,
                                                                  expanded_expr[1])))
      {
        return AST::Fragment::create_error();
      }
  ...
  MacroBuiltin::include_handler(Location invoc_locus, AST::MacroInvocData &invoc)
  {
    ...
    std::string filename = source_relative_path(lit_expr->as_string(), invoc_locus);
    auto target_filename = Rust::Session::get_instance().include_extra_file(std::move(filename));
  ...
  {
    auto current_line = Session::get_instance().linemap->location_to_line(invoc_locus);

    auto line_no = AST::SingleASTNode(std::unique_ptr<AST::Expr>(
        new AST::LiteralExpr(std::to_string(current_line), AST::Literal::INT,
                             PrimitiveCoreType::CORETYPE_U32, {}, invoc_locus)));
    auto tok = make_token(Token::make_int(invoc_locus, std::to_string(current_line)));
  ...
  MacroBuiltin::option_env_handler(Location invoc_locus, AST::MacroInvocData &invoc)
  {
    ...
    if (expanded_expr.size() < 1 || expanded_expr.size() > 2)
    {
      rust_error_at(invoc_locus, "OptionEnv! takes 1 argument");
      return AST::Fragment::create_error();
    }
    if (expanded_expr.size() > 0)
    {
