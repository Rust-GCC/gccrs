...
{
  auto hash
    = builder.path_in_expression ({builder.get_path_start(), "hash", "Hash", "hash"}, true);
...
DeriveHash::hash_fn (std::unique_ptr<BlockExpr> &&block)
{
  ...
  auto bounds = vec (
    builder.trait_bound (builder.type_path ({builder.get_path_start(), "hash", "Hasher"}, true)));
  auto generics = vec (
...
{
  auto hash_path = builder.type_path ({builder.get_path_start(), "hash", "Hash"}, true);
