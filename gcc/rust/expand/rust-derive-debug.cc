...
DeriveDebug::stub_debug_fn ()
{
  ...
  auto return_type
    = ptrify (builder.type_path ({builder.get_path_start(), "fmt", "Result"}, true));

  auto mut_fmt_type_inner
    = ptrify (builder.type_path ({builder.get_path_start(), "fmt", "Formatter"}, true));
  ...
  const std::vector<std::unique_ptr<GenericParam>> &type_generics)
{
  ...
  auto debug = builder.type_path ({builder.get_path_start(), "fmt", "Debug"}, true);
  auto generics
