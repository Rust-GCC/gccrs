	    ...
	    const std::string &trait)
{
  auto formatter_fn = std::unique_ptr<AST::Expr> (new AST::PathInExpression (
    builder.path_in_expression ({builder.get_path_start(), "fmt", trait, "fmt"})));

  auto path = std::unique_ptr<AST::Expr> (new AST::PathInExpression (
    builder.path_in_expression ({builder.get_path_start(), "fmt", "ArgumentV1", "new"})));
		    ...
		    std::vector<std::unique_ptr<AST::Token>> &&tokens)
{
  ...
  auto final_path = std::make_unique<AST::PathInExpression> (
    builder.path_in_expression ({builder.get_path_start(), "fmt", "Arguments", "new_v1"}));
