...
TyTy::BaseType *TypeCheckExpr::Resolve(HIR::Expr &expr) {
...
void TypeCheckExpr::visit(HIR::TupleIndexExpr &expr) {
  ...
  if (resolved->get_kind() == TyTy::TypeKind::TUPLE) {
    ...
    if (field_tyty == nullptr) {
      rust_error_at(expr.get_locus(), "failed to lookup field type at index %i",
                    index);
      return;
    }
  ...
  if (!adt->is_tuple_struct()) {
    rust_error_at(expr.get_locus(), "expected tuple or tuple struct, found %qs",
                  adt->get_name().c_str());
    return;
  }
...
void TypeCheckExpr::visit(HIR::TupleExpr &expr) {
  if (expr.is_unit()) {
...
void TypeCheckExpr::visit(HIR::ReturnExpr &expr) {
  ...
  location_t expr_locus =
      expr.has_return_expr() ? expr.get_expr().get_locus() : expr.get_locus();
...
void TypeCheckExpr::visit(HIR::CallExpr &expr) {
...
void TypeCheckExpr::visit(HIR::AssignmentExpr &expr) {
  infered = TyTy::TupleType::get_unit_type();
...
void TypeCheckExpr::visit(HIR::CompoundAssignmentExpr &expr) {
  infered = TyTy::TupleType::get_unit_type();
  ...
  if (!valid) {
    rust_error_at(expr.get_locus(),
                  ...
                  expr.get_operator_str().c_str(), lhs->as_string().c_str(),
                  rhs->as_string().c_str());
    return;
  }
}

void TypeCheckExpr::visit(HIR::LiteralExpr &expr) {
  infered = resolve_literal(expr.get_mappings(), expr.get_literal(),
                            expr.get_locus());
}

void TypeCheckExpr::visit(HIR::ArithmeticOrLogicalExpr &expr) {
  auto lhs = TypeCheckExpr::Resolve(expr.get_lhs());
  auto rhs = TypeCheckExpr::Resolve(expr.get_rhs());
  ...
  case ArithmeticOrLogicalOperator::RIGHT_SHIFT: {
    ...
    infered =
        cast_site(expr.get_mappings().get_hirid(), from, to, expr.get_locus());
  } break;
...
void TypeCheckExpr::visit(HIR::ComparisonExpr &expr) {
  auto lhs = TypeCheckExpr::Resolve(expr.get_lhs());
  auto rhs = TypeCheckExpr::Resolve(expr.get_rhs());

  auto borrowed_rhs = new TyTy::ReferenceType(
      mappings.get_next_hir_id(), TyTy::TyVar(rhs->get_ref()), Mutability::Imm);
  ...
  bool operator_overloaded = resolve_operator_overload(
      lang_item_type, expr, lhs, borrowed_rhs, segment);
  if (operator_overloaded)
    return;
...
void TypeCheckExpr::visit(HIR::LazyBooleanExpr &expr) {
  auto lhs = TypeCheckExpr::Resolve(expr.get_lhs());
  auto rhs = TypeCheckExpr::Resolve(expr.get_rhs());
  ...
  lhs = unify_site(
      ...
      TyTy::TyWithLocation(boolean_node, expr.get_lhs().get_locus()),
      TyTy::TyWithLocation(lhs, expr.get_lhs().get_locus()), expr.get_locus());

  rhs = unify_site(
      expr.get_mappings().get_hirid(),
      TyTy::TyWithLocation(boolean_node, expr.get_rhs().get_locus()),
      TyTy::TyWithLocation(rhs, expr.get_rhs().get_locus()), expr.get_locus());
...
void TypeCheckExpr::visit(HIR::NegationExpr &expr) {
  ...
  bool operator_overloaded =
      resolve_operator_overload(lang_item_type, expr, negated_expr_ty, nullptr);
  if (operator_overloaded)
    return;
  ...
  case NegationOperator::NEGATE: {
    bool valid = (negated_expr_ty->get_kind() == TyTy::TypeKind::INT) ||
  ...
  case NegationOperator::NOT: {
    bool valid = (negated_expr_ty->get_kind() == TyTy::TypeKind::BOOL) ||
...
void TypeCheckExpr::visit(HIR::IfExpr &expr) {
  ...
  unify_site(
      expr.get_mappings().get_hirid(), TyTy::TyWithLocation(bool_ty),
      TyTy::TyWithLocation(cond_type, expr.get_if_condition().get_locus()),
      expr.get_locus());
  ...
  infered = coercion_site(
      expr.get_mappings().get_hirid(), TyTy::TyWithLocation(unit_ty),
      TyTy::TyWithLocation(block_type, expr.get_if_block().get_locus()),
      expr.get_locus());
}

void TypeCheckExpr::visit(HIR::IfExprConseqElse &expr) {
  TyTy::BaseType *bool_ty = nullptr;
  bool ok = context->lookup_builtin("bool", &bool_ty);
  rust_assert(ok);
  ...
  if (if_blk_resolved->get_kind() == TyTy::NEVER)
    ...
    infered = unify_site(
        expr.get_mappings().get_hirid(),
        TyTy::TyWithLocation(if_blk_resolved, expr.get_if_block().get_locus()),
...
void TypeCheckExpr::visit(HIR::UnsafeBlockExpr &expr) {
  infered = TypeCheckExpr::Resolve(expr.get_block_expr());
}

void TypeCheckExpr::visit(HIR::BlockExpr &expr) {
  if (expr.has_label())
  ...
  for (auto &s : expr.get_statements()) {
    ...
    if (s->is_unit_check_needed() && !resolved->is_unit()) {
      ...
      unify_site(s->get_mappings().get_hirid(), TyTy::TyWithLocation(unit),
...
void TypeCheckExpr::visit(HIR::AnonConst &expr) {
  if (!expr.is_deferred()) {
    infered = TypeCheckExpr::Resolve(expr.get_inner_expr());
    return;
  }

  TyTy::TyVar var = TyTy::TyVar::get_implicit_const_infer_var(expr.get_locus());
  infered = var.get_tyty();
}

void TypeCheckExpr::visit(HIR::ConstBlock &expr) {
  infered = TypeCheckExpr::Resolve(expr.get_const_expr());
}

void TypeCheckExpr::visit(HIR::RangeFromToExpr &expr) {
  ...
  bool ok = context->lookup_type(item->get_mappings().get_hirid(), &item_type);
  rust_assert(ok);
...
void TypeCheckExpr::visit(HIR::RangeFromExpr &expr) {
...
void TypeCheckExpr::visit(HIR::RangeToExpr &expr) {
...
void typecheck_inline_asm_operand(HIR::InlineAsm &expr) {
...
void TypeCheckExpr::visit(HIR::InlineAsm &expr) {
...
void TypeCheckExpr::visit(HIR::LlvmInlineAsm &expr) {
...
void TypeCheckExpr::visit(HIR::OffsetOf &expr) {
...
void TypeCheckExpr::visit(HIR::RangeFullExpr &expr) {
...
void TypeCheckExpr::visit(HIR::RangeFromToInclExpr &expr) {
...
void TypeCheckExpr::visit(HIR::ArrayIndexExpr &expr) {
  ...
  bool maybe_simple_array_access =
      types_compatable(TyTy::TyWithLocation(index_expr_ty),
                       TyTy::TyWithLocation(size_ty), expr.get_locus(), false);
  if (maybe_simple_array_access &&
      direct_array_expr_ty->get_kind() == TyTy::TypeKind::ARRAY) {
    unify_site(
        expr.get_index_expr().get_mappings().get_hirid(),
        TyTy::TyWithLocation(size_ty),
        TyTy::TyWithLocation(index_expr_ty, expr.get_index_expr().get_locus()),
        expr.get_locus());
  ...
  bool operator_overloaded = resolve_operator_overload(
      lang_item_type, expr, array_expr_ty, index_expr_ty);
  if (operator_overloaded) {
...
void TypeCheckExpr::visit(HIR::ArrayExpr &expr) {
  ...
  case HIR::ArrayElems::ArrayExprType::COPIED: {
    ...
    auto capacity_expr_ty = TypeCheckExpr::Resolve(elems.get_num_copies_expr());
  ...
  case HIR::ArrayElems::ArrayExprType::VALUES: {
    ...
    for (auto &type : types) {
      element_type = unify_site(
          expr.get_mappings().get_hirid(), TyTy::TyWithLocation(element_type),
          TyTy::TyWithLocation(type, type->get_locus()), expr.get_locus());
    }
    ...
    capacity_expr = new HIR::LiteralExpr(
        mapping, capacity_str, HIR::Literal::LitType::INT,
        PrimitiveCoreType::CORETYPE_USIZE, UNDEF_LOCATION, {});
  ...
  tree capacity_value = Compile::HIRCompileBase::query_compile_const_expr(
      ctx, capacity_type, *capacity_expr);
  ...
  auto const_type = new TyTy::ConstValueType(
      capacity_value, expected_ty, capacity_expr_id, capacity_expr_id);
  ...
  infered =
      new TyTy::ArrayType(expr.get_mappings().get_hirid(), expr.get_locus(),
                          TyTy::TyVar(const_type->as_base_type()->get_ty_ref()),
                          TyTy::TyVar(element_type->get_ref()));
}

// empty struct
void TypeCheckExpr::visit(HIR::StructExprStruct &struct_expr) {
...
void TypeCheckExpr::visit(HIR::StructExprStructFields &struct_expr) {
  infered = TypeCheckStructExpr::Resolve(struct_expr);
}

void TypeCheckExpr::visit(HIR::GroupedExpr &expr) {
  infered = TypeCheckExpr::Resolve(expr.get_expr_in_parens());
}

void TypeCheckExpr::visit(HIR::FieldAccessExpr &expr) {
  ...
  bool found =
      vaiant->lookup_field(expr.get_field_name().as_string(), &lookup, nullptr);
...
bool is_default_fn(const MethodCandidate &candidate) {
...
void emit_ambiguous_resolution_error(HIR::MethodCallExpr &expr,
                                     std::set<MethodCandidate> &candidates) {
  rich_location r(line_table, expr.get_method_name().get_locus());
  std::string rich_msg =
      "multiple " + expr.get_method_name().get_segment().as_string() + " found";
...
void TypeCheckExpr::visit(HIR::MethodCallExpr &expr) {
  ...
  HirId autoderef_mappings_id = expr.get_receiver().get_mappings().get_hirid();
  ...
  if (resolved_candidate.is_impl_candidate()) {
    ...
    infer_arguments.get_mut_regions() = fn->get_used_arguments().get_regions();
    HIR::ImplBlock &impl = *resolved_candidate.item.impl.parent;
    TyTy::BaseType *impl_self_infer =
        TypeCheckItem::ResolveImplBlockSelfWithInference(impl, expr.get_locus(),
                                                         &infer_arguments);
  ...
  if (expr.get_method_name().has_generic_args()) {
    ...
    lookup =
        SubstMapper::Resolve(lookup, expr.get_method_name().get_locus(), &args);
    if (lookup->get_kind() == TyTy::TypeKind::ERROR)
      return;
  } else if (lookup->needs_generic_substitutions()) {
    rust_debug("method needs inference: {%s}", lookup->debug_str().c_str());
    lookup =
        SubstMapper::InferSubst(lookup, expr.get_method_name().get_locus());
  }

  rust_debug("type-checking method_call: {%s}", lookup->debug_str().c_str());

  TyTy::BaseType *function_ret_tyty = TyTy::TypeCheckMethodCallExpr::go(
      static_cast<TyTy::FnType *>(lookup), expr, adjusted_self, context);
  if (function_ret_tyty == nullptr ||
      function_ret_tyty->get_kind() == TyTy::TypeKind::ERROR) {
    rust_error_at(expr.get_locus(), "failed to lookup type to MethodCallExpr");
    return;
  }
...
void TypeCheckExpr::visit(HIR::LoopExpr &expr) {
  context->push_new_loop_context(expr.get_mappings().get_hirid(),
                                 expr.get_locus());
...
void TypeCheckExpr::visit(HIR::BreakExpr &expr) {
  ...
  if (expr.has_break_expr()) {
    TyTy::BaseType *break_expr_tyty = TypeCheckExpr::Resolve(expr.get_expr());

    TyTy::BaseType *loop_context = context->peek_loop_context();
    if (loop_context->get_kind() == TyTy::TypeKind::ERROR) {
      rust_error_at(expr.get_locus(), ErrorCode::E0571,
                    "can only %<break%> with a value inside a %<loop%> block");
      return;
    }

    TyTy::BaseType *unified_ty = unify_site(
        expr.get_mappings().get_hirid(), TyTy::TyWithLocation(loop_context),
        TyTy::TyWithLocation(break_expr_tyty, expr.get_expr().get_locus()),
        expr.get_locus());
...
void TypeCheckExpr::visit(HIR::ContinueExpr &expr) {
  if (!context->have_loop_context()) {
    rust_error_at(expr.get_locus(), ErrorCode::E0268,
...
void TypeCheckExpr::visit(HIR::BorrowExpr &expr) {
...
void TypeCheckExpr::visit(HIR::DereferenceExpr &expr) {
  TyTy::BaseType *resolved_base = TypeCheckExpr::Resolve(expr.get_expr());
...
void TypeCheckExpr::visit(HIR::TypeCastExpr &expr) {
...
void TypeCheckExpr::visit(HIR::MatchExpr &expr) {
  ...
  for (auto &kase : expr.get_match_cases()) {
    ...
    for (auto &pattern : kase_arm.get_patterns()) {
      ...
      TyTy::BaseType *checked_kase =
          unify_site(expr.get_mappings().get_hirid(),
                     TyTy::TyWithLocation(
                         scrutinee_tyty, expr.get_scrutinee_expr().get_locus()),
  ...
  for (size_t i = 1; i < kase_block_tys.size(); i++) {
    ...
    infered = coercion_site(expr.get_mappings().get_hirid(),
                            TyTy::TyWithLocation(infered),
                            TyTy::TyWithLocation(kase_ty), expr.get_locus());
  }
}

void TypeCheckExpr::visit(HIR::ClosureExpr &expr) {
  ...
  if (context->have_function_context()) {
    ...
    ident =
        RustIdent{current_context_fndecl->get_ident().path, expr.get_locus()};
  ...
  TyTy::TupleType *closure_args =
      new TyTy::TupleType(implicit_args_id, expr.get_locus(), parameter_types);
  ...
  coercion_site(expr.get_mappings().get_hirid(),
                TyTy::TyWithLocation(result_type.get_tyty(), result_type_locus),
  ...
  HIR::TupleType *implicit_tuple = new HIR::TupleType(
      mapping, {} // we dont need to fill this out because it will
...
bool TypeCheckExpr::resolve_operator_overload(
  ...
  for (auto &c : candidates) {
    ...
    const TyTy::FnType &fn = *static_cast<const TyTy::FnType *>(candidate_type);
  ...
  if (selected_candidates.size() > 1) {
    ...
    TyTy::SubstitutionArgumentMappings arg_mappings(
        mappings, binding_args, TyTy::RegionParamList(trait_subst.size()),
        expr.get_locus());
    pred.apply_argument_mappings(arg_mappings, false);

    infer->inherit_bounds({pred});
    DeferredOpOverload defer(expr.get_mappings().get_hirid(), lang_item_type,
                             specified_segment, pred, expr);
    context->insert_deferred_operator_overload(std::move(defer));

    if (rhs != nullptr)
      lhs =
          unify_site(expr.get_mappings().get_hirid(), TyTy::TyWithLocation(lhs),
  ...
  if (current_context.get_type() == TypeCheckContextItem::ItemType::IMPL_ITEM) {
    ...
    bool is_deref_match =
        fn->get_function_name().as_string().compare(
            LangItem::ToString(LangItem::Kind::DEREF)) == 0 ||
        fn->get_function_name().as_string().compare(
            LangItem::ToString(LangItem::Kind::DEREF_MUT)) == 0;

    bool is_recursive_op = fn->get_function_name().as_string().compare(
                               associated_item_name) == 0 ||
    ...
    if (parent->has_trait_ref() && is_recursive_op) {
      ...
      if (!trait_reference->is_error()) {
        TyTy::BaseType *lookup = nullptr;
        bool ok = context->lookup_type(fn->get_mappings().get_hirid(), &lookup);
        rust_assert(ok);
        ...
        bool is_lang_item_impl = trait_reference->get_mappings().get_defid() ==
...
bool TypeCheckExpr::resolve_fn_trait_call(HIR::CallExpr &expr,
  ...
  rust_debug_loc(expr.get_locus(), "resolved call-expr to fn trait: {%u} {%s}",
                 candidate.candidate.ty->get_ref(),
                 candidate.candidate.ty->debug_str().c_str());
  ...
  TyTy::BaseType *function_ret_tyty = TyTy::TypeCheckMethodCallExpr::go(
      fn, expr.get_mappings(), args, expr.get_locus(), expr.get_locus(),
...
bool TypeCheckExpr::validate_arithmetic_type(
