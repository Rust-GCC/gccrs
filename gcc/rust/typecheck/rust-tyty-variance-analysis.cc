...
void
add_type_constraints (ADTType &type)
{
  GenericTyVisitorCtx::get ().process_type (type);
}

void
solve ()
{
  auto ctx = GenericTyVisitorCtx::get ();
  ctx.solve ();
  ctx.debug_print_solutions ();
}

std::vector<Variance>
query_generic_variance (const ADTType &type)
{
  auto ctx = GenericTyVisitorCtx::get ();
  auto solution_index = ctx.lookup_type_index (type.get_orig_ref ());
  ...
  for (size_t i = 0; i < num_lifetimes + num_types; ++i)
    {
      result.push_back (ctx.solutions[solution_index.value () + i]);
    }

  return result;
}
std::vector<Variance>
query_type_variances (BaseType *type)
{
  TyVisitorCtx ctx;
...
GenericTyVisitorCtx *GenericTyVisitorCtx::instance = nullptr;

GenericTyVisitorCtx &
GenericTyVisitorCtx::get ()
{
  if (instance == nullptr)
    instance = new GenericTyVisitorCtx ();

  return *instance;
}

void
GenericTyVisitorCtx::process_type (ADTType &type)
{
  rust_debug ("add_type_constraints: %s", type.as_string ().c_str ());

  first_lifetime = lookup_or_add_type (type.get_orig_ref ());
  first_type
    = first_lifetime + type.get_used_arguments ().get_regions ().size ();

  for (const auto &param : type.get_substs ())
...
void
GenericTyVisitorCtx::debug_print_solutions ()
{
...
GenericTyVisitorCtx::lookup_type_index (HirId orig_ref)
{
...
std::string
GenericTyVisitorCtx::to_string (const Term &term) const
{
  switch (term.kind)
    {
    case Term::CONST:
...
std::string
GenericTyVisitorCtx::to_string (SolutionIndex index) const
{
  // Search all values in def_id_to_solution_index_start and find key for
  // largest value smalled than index
...
Variance
GenericTyVisitorCtx::evaluate (Term *term)
{
  switch (term->kind)
    {
    case Term::CONST:
...
GenericTyVisitorCtx::lookup_or_add_type (HirId hir_id)
{
  ...
  auto index = lookup_type_index (hir_id);
  if (index.has_value ())
    {
      return index.value ();
    }
  ...
  auto solution_index = solutions.size ();
  map_from_ty_orig_ref.emplace (ty->get_orig_ref (), solution_index);
  ...
  for (size_t i = 0; i < num_lifetime_param + num_type_param; ++i)
    solutions.emplace_back (Variance::bivariant ());
...
GenericTyVisitorCtx::add_constraints_from_ty (BaseType *type, Term variance)
{
  rust_debug ("\tadd_constraint_from_ty: %s with v=%s",
	      type->as_string ().c_str (), to_string (variance).c_str ());
...
{
  rust_debug ("\t\tadd_constraint: %s", to_string (term).c_str ());

  if (term.kind == Term::CONST)
    {
      // Constant terms do not depend on other solutions, so we can
      // immediately apply them.
      solutions[index].join (term.const_val);
    }
  else
    {
      constraints.push_back ({index, new Term (term)});
    }
}
  ...
  for (size_t i = 0; i < num_lifetimes + num_types; ++i)
    {
      ...
      auto variance_i
	= invariant_args
	    ? Term::make_transform (variance, V::invariant ())
...
{
  return Term::make_transform (variance, V::contravariant ());
}
...
{
  // Handle function
  auto variances = query_generic_variance (*lookup_type (ref)->as<ADTType> ());
