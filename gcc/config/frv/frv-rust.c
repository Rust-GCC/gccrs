...
frv_rust_target_cpu_info (void)
{
  ...
  // TODO: maybe option on branch cost, but wouldn't work well as "define only"
  // option
  if (TARGET_COND_EXEC)
    rust_add_target_info ("target_feature", "cond-exec");
  /* TODO: maybe option on cond-exec-insns, but wouldn't work well as
   * "define-only" option. also cond-exec-temps, sched-lookahead */
  if (TARGET_COND_MOVE)
  ...
  // TODO: maybe something with no-eflags if possible? tomcat-stats?
