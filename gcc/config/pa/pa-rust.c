...
void
pa_rust_target_cpu_info (void)
{
  rust_add_target_info ("target_arch", "hppa");

  /* names made up by me (as no apparent current nor historical llvm support),
   * based on gcc options */
  /* TODO: figure out how to get data for linker-opt, nosnake - not defined in
   * variable, apparently */
  ...
  /* TODO: figure out how to represent fixed-range (ranges of registers to make
   * fixed) as define */
  if (TARGET_GAS)
  ...
  /* defines for generating PA 1.1 or PA 2.0 code - TODO should PA 2.0 imply
   * PA 1.1 as well? */
  if (TARGET_PA_11)
