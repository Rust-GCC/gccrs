...
void
nvptx_rust_target_cpu_info (void)
{
  if (TARGET_ABI64)
  ...
  /* TODO: should this also override target_os and target_vendor to be "cuda"
   * and "nvidia"? */

  // names derived from llvm
  switch (ptx_isa_option)
    {
    /* TODO: if gcc adds other sm versions (llvm has 20, 21, 32, 37, 50, 52, 53,
     * 60, 61, 62, 70, 72, 75, 80 as well), add them here  */
    case PTX_ISA_SM30:
  ...
  /* TODO: add ptx versions as separate features if gcc adds them (ptx32, 40,
   * 41, 42, 43, 50, 60, 61, 63, 64, 65, 70)  */

  /* NOTE: below are all gcc-derived features that do not appear in llvm. they
   * appeared useful, so added */
  if (nvptx_optimize)
