...
static void
mips3_target_info (void)
{
  rust_add_target_info ("target_feature", "mips3");
  mips2_target_info ();
  mips3_32_target_info ();
  mips3_32r2_target_info ();
  // maybe always have FeatureGP64Bit and FeatureFP64Bit? or maybe make them
  // prereqs?
}
...
static void
mips32r6_target_info (void)
{
  rust_add_target_info ("target_feature", "mips32r6");
  mips32r5_target_info ();
  // maybe also hardcode having FeatureFP64Bit, FeatureNaN2008, and
  // FeatureAbs2008, or have prereqs
}

/* Add target info for MIPS64 ISA support.  */

static void
mips64_target_info (void)
{
...
static void
mips64r6_target_info (void)
{
  rust_add_target_info ("target_feature", "mips64r6");
  mips64r5_target_info ();
  mips32r6_target_info ();
  // maybe hardcode FeatureNaN2008 and FeatureAbs2008, or just have them as
  // prereqs?
}
...
void
mips_rust_target_cpu_info (void)
{
  if (TARGET_64BIT)
  ...
  // TODO maybe implement more features that aren't in llvm but are in gcc at
  // some point?
  if (!(TARGET_ABICALLS))
  ...
  // TODO: find out what use-tcc-in-div ("force the assembler to use trapping")
  // corresponds to
  if (!mips_madd4)
  ...
  /* TODO: find out what use-indirect-jump-hazard corresponds with ("use
   * indirect jump guards to prevent certain speculation based attacks") */
}
