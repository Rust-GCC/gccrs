...
void microblaze_rust_target_cpu_info(void) {
    rust_add_target_info("target_arch", "microblaze");

    // llvm has no current or historical support for microblaze so I made up names and features
    ...
    // TODO find if value for xl-mode-xilkernel option is actually saved (it doesn't appear to be)
}
