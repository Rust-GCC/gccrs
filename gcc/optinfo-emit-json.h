...
public:
  ...
  get_id_value_for_pass (const opt_pass &pass);

  std::unique_ptr<json::object>
  pass_to_json (const opt_pass &pass);

  std::unique_ptr<json::array>
  inlining_chain_to_json (location_t loc);

  std::unique_ptr<json::object>
  optinfo_to_json (const optinfo &optinfo);

  void
  add_pass_list (json::array *arr, const opt_pass *pass);
